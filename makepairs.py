...
def find_pairs(dataframe):
    ...
    pairs = pd.DataFrame(data =[],columns =['file1','file2','identity_no','match'])

    pair_count = 0
    for row in range(len(dataframe)):
        identity_val = dataframe['identity_no'].iloc[row]
        pair_list = dataframe.Filename[dataframe.identity_no == identity_val].values
        if len(pair_list) >= 2:
            for f1,f2 in itertools.combinations(pair_list,r= 2):
                pairs = pairs.append({'file1':f1,'file2':f2,'identity_no':identity_val,'match':'Yes'},ignore_index = True)
                # pair_count += 1
                # if pair_count % 500 == 0:
                #     print(pair_count)
